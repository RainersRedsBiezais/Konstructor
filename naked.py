...
#Prints NASA request responses data  
...
if r.status_code == 200:
	...
	#Defining arrays for safe and hazardous asteroids
	ast_safe = []
	ast_hazardous = []
	#Checks if the json file has the right structure that it can process, checks if the element exists  
	if 'element_count' in json_data:
		ast_count = int(json_data['element_count']) #converts the count to int 
		print("Asteroid count today: " + str(ast_count))

		if ast_count > 0: # If bigger then 0 then the data gets processed 
			for val in json_data['near_earth_objects'][request_date]:
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
					tmp_ast_name = val['name'] # Getting the name of the asteroid 
					tmp_ast_nasa_jpl_url = val['nasa_jpl_url'] # Getting the url of the asteroid 
					if 'kilometers' in val['estimated_diameter']: # Getting the min and max diameter for asteroid 
					...
					if len(val['close_approach_data']) > 0: #Gets multiple values about the asteroid (Date,velocity,speed,miss distance,etc.)
						if 'epoch_date_close_approach' and 'relative_velocity' and 'miss_distance' in val['close_approach_data'][0]:
							tmp_ast_close_appr_ts = int(val['close_approach_data'][0]['epoch_date_close_approach']/1000)
							tmp_ast_close_appr_dt_utc = datetime.utcfromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
							tmp_ast_close_appr_dt = datetime.fromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
							# Gets how fast it is going
					...
					if tmp_ast_hazardous == True: # Adds to array if it is hazardous, according to the previous structure
					...
					else: # Adds to the safe asteroid array
	...
	if len(ast_hazardous) > 0:
		# Sorts by time (how close it is to Earth from closest to farthest)
		...
		# Sorts by the distance, how close they will pass the Earth 
